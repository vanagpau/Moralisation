...
# Normality: Shaipro-Wilk test per variable mean per session and plot histograms of each distribution
...
if (tmp$p.value < .05) {print(tmp)
  print(ggplot(df[df$session == i,var_names4[j]], aes(x=get(var_names4[j]))) +
        geom_histogram(binwidth = .5) + ggtitle(paste0("Session ",i)) +
  xlab(var_names4[j]))}
else  print("IS NORMALLY DISTRIBUTED")
}
}


# Normality QQ plots as above
