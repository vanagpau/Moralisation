...
df_sim$PID <- as.numeric(df_sim$PID)
df_sim$session <- as.numeric(df_sim$session)

m_sim_null <- lmer(moral ~ session|PID, data = df_sim)
m_sim <- lmer(moral ~ pig + (session|PID), data = df_sim)

library(simr)
library(mixedpower)

mixedpower(m_sim, data = df_sim, fixed_effects = c("pig"), simvar = "PID", n_sim = 100, steps = c(85, 400, 1000), critical_value = 2)


# simr::powerSim(fit = m_sim, nsim = 100)
# 
