...
wide_df <- df %>% pivot_wider(id_cols = PID, names_from = session, values_from = c(pig, emo, moral, cog, hed, diss, att, emo_obj, cog_delta, emo_delta, pig_delta, moral_delta))

# METHOD 1. using CHANGE SCORES
...
# METHOD 2: using timepoints

m2.2 <- lm(moral_4 ~ moral_1 + pig_4 + emo_4, data = wide_df)
summary(m2.2)
...
# Also no relationship with change in Cognitive Dissonance or Hedonic Motivation
