...
fn_var <- function(d, s1, s2 = NULL, s3 = NULL) {
 nam <- paste0(d, s1, s2, s3)
 print(nam)
 df_temp <<- get(d) %>% group_by(`PID`) %>% filter(str_detect(`Question Key`, s1) == TRUE) %>% mutate(m = mean(Response)) %>% select(PID, session, m) %>% distinct()
 if (nrow(df_temp) == 0) return(NULL)
 assign(nam, df_temp, envir = .GlobalEnv)
}

lapply(dflist, fn_var, "att")



for (i in 1:length(var_names)) {
df_temp2 <- lapply(dflist, fn_var, s1 = var_names[i] )
print (nrow(df_temp2) ) 
}


lapply(dflist, fn_var, s1 = "hed")
 fn_var("n_l8iw", "hed")


n_4kh5 %>% group_by(`PID`) %>% filter(str_detect(`Question Key`, "cog") == TRUE) %>% mutate(moral_cognition = mean(Response)) %>% select(PID, session, moral_cognition) %>% distinct()

n_4kh5 %>% group_by(`PID`) %>% filter(str_detect(`Question Key`, "att") == TRUE) %>% mutate(attitude = mean(Response))
n_4kh5 %>% group_by(`PID`) %>% filter(str_detect(`Question Key`, "pig") == TRUE) %>% mutate(piggybacking = mean(Response))
