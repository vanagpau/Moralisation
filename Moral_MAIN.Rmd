...
n_auod %>% filter(`Question Key` != "END QUESTIONNAIRE" &  `Question Key` != "BEGIN QUESTIONNAIRE", !grepl("quantised", n_auod$`Question Key`)  ) %>% select(`Participant Private ID`, `Question Key`, Response) %>% group_by(`Participant Private ID`) %>% summarise (HRPS = mean(Response))
