...
AIC(m1)
apa.reg.table(m1, filename = "m1.doc")

# re-run regression models using session 4 moralisation as DV and session 1 as a co-variate

# This shows that moralisation at session 1 is significant predictor of moralisation at session 4

wide_df <- df %>% pivot_wider(id_cols = PID, names_from = session, values_from = c(pig, emo, moral, pig_delta, emo_delta, moral_delta, cog_delta))

m1.1 <- lm(moral_4 ~ moral_1, data = wide_df)
summary(m1.1)
performance::check_model(m1.1)
AIC(m1.1)
apa.reg.table(m1.1, filename = "m1_1.doc")


# When model1 is replicated using this method, emotions and piggybacking still significant

m1.2 <- lm(moral_4 ~ moral_1 + pig_delta_1 + emo_delta_1, data = wide_df)
summary(m1.2)
performance::check_model(m1.2)
AIC(m1.2)
apa.reg.table(m1.2, filename = "m1_2.doc")
...
AIC(m1.3)
apa.reg.table(m1.3, filename = "m1_3.doc")
...
m5 <- lm(moral ~ emo + att, df)
summary(m5)
performance::check_model(m5)
AIC(m5)

m6 <- lm(moral ~ pig + att, df)
summary(m6)
performance::check_model(m6)
AIC(m6)

m7 <- lm(moral ~ emo + att + pig, df)
summary(m7)
performance::check_model(m7)
AIC(m7)

m8 <- lm(moral ~ emo + pig, df)
summary(m8)
...
m9 <- lm(moral ~ emo + pig + hed, df)
summary(m9)
performance::check_model(m9)
AIC(m9)

# Moral cognitions is significant by this measure and absorbs some Moral Emotions variance. m10 AIC score lower than m4
m10 <- lm(moral ~ emo + pig + cog, df)
summary(m10)
performance::check_model(m10)
AIC(m10)

# Run model with cog + pig only -> AIC still  worse than m10
m10.1 <- lm(moral ~ pig + cog, df)
summary(m10.1)
performance::check_model(m10.1)
AIC(m10.1)
...
# Code to add significance data to correlation matrix
...
corrplot::corrplot(c1, method = "circle", type = "upper", order = "FPC", col = colorRampPalette(c("red", "white", "blue"))(20), p.mat = p.mat, sig.level = 0.05, addCoef.col = "black", tl.col="black", tl.srt=45, tl.cex = .75, insig = "blank", number.font = 1)



apaTables::apa.cor.table(c1, filename = "corrtable.doc")

# Do moral piggybacking and moral cognitions mediate the other variables?

#model direct effect (IV on DV)
direct.m1 <- lm(moral ~ pig + cog, df)
summary(direct.m1)

#model effect of IV's on mediator variable 1
step1.m1 <- lm(emo ~ pig + cog, df) 
summary(step1.m1)

# model effect of mediator variable on DV, controlling for 
step2.m1 <- lm(moral ~ pig + cog + emo, df)
summary(step2.m1)
...
med1 <- mediation::mediate(step1.m1, step2.m1, treat=c('pig', 'cog'), mediator='emo', boot=T)
summary(med1)
