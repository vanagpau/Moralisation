...
# Attitude non-significant
m5 <- lm(moral ~ emo + pig + att, df)
summary(m5)
performance::check_model(m5)
AIC(m5)

# Hedonic Motivation non-significant
m6 <- lm(moral ~ emo + pig + hed, df)
summary(m6)
performance::check_model(m6)
AIC(m6)

# Dissonance Reduction - non-significant
m7 <- lm(moral ~ emo + pig + diss, df)
summary(m7)
performance::check_model(m7)
AIC(m7)

# Moral cognitions is significant by this measure and absorbs some Moral Emotions variance. m8 AIC score lower than m4
m8 <- lm(moral ~ emo + pig + cog, df)
summary(m8)
performance::check_model(m8)
AIC(m8)

# People Meotions non-significant
m9 <- lm(moral ~ emo + pig + emo_obj, df)
summary(m9)
...
# IV = hed; mod = emo + pig
med1 <- psych::mediate('Moralisation', 'Hedonic Motivation', m = c('Moral Emotions', 'Moral Piggybacking'), data = df, main = "c = Direct Effect; c' = Indirect Effect")
summary(med1)

# IV = hed; mod = emo + pig
med2 <- psych::mediate('moral', 'att', m = c('emo', 'pig'), data = df)
summary(med2)

# IV = diss; mod = emo + pig
med3 <- psych::mediate('moral', 'diss', m = c('emo', 'pig'), data = df)
summary(med3)

# IV = emo_obj; mod = emo + pig
med4 <- psych::mediate('moral', 'emo_obj', m = c('emo', 'pig'), data = df)
summary(med4)


mediate.diagram(med1, main = "KEY: hed = Hedonic Motivation, emo = Moral Emotions\npig = Moral Piggybacking, moral = Moralisation")
mediate.diagram(med2, main = "KEY: att = Attitude")
mediate.diagram(med3, main = "KEY: diss = Dissonance Reduction Strategies")
mediate.diagram(med4, main = "KEY: emo_obj = People Emotions")



# Note that emo/emo_obj shows a different result when emo is the mediator..
med5 <- psych::mediate('moral', 'emo', m = c('emo_obj', 'pig'), data = df)
summary(med5)
