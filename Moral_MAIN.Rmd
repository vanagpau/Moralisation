...
df %>% select(PID, session, var_names4) %>% 
  ...
  ggplot(aes(x = session, y = value)) + geom_point(aes(color = variable), size = 3) +
  geom_line(aes(color = variable), size = 1.3) +
  scale_color_brewer(palette = "Spectral", labels = c("Attitude", "Moral Cognitions", "Dissonance Reduction",
...
#Create rebased chart (with error bars) - NOT WORKING YET
# df %>% select(PID, session, var_names4) %>% 
#   pivot_longer(cols = !PID & !session, names_to = "variable", values_to = "value") %>%
#   group_by(session, variable)  %>% mutate(sd = sd(value)) %>%
#   mutate(mean = mean(value)) %>%  unique() %>% cbind(session1) %>%
#   transmute(value = value - ...4) %>%
#   ggplot(aes(x = session, y = value)) + geom_point(aes(color = variable), size = 3) +
#   geom_line(aes(color = variable), size = 1.3) +
#   scale_color_brewer(palette = "Spectral", labels = c("Attitude", "Moral Cognitions", "Dissonance Reduction",
