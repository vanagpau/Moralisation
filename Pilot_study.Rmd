...
df2 %>% filter(`Zone Name` != "slider") %>% group_by(`Spreadsheet Row`, PID, `Screen Name`) %>% summarise(R = mean(Response)) %>% pivot_wider(id_cols = PID, names_from = c(`Screen Name`,`Spreadsheet Row`), values_from = R) 

df2 %>% filter(`Spreadsheet Row` == "Meat", `Zone Name` != "slider") %>% t.test(Response ~ `Screen Name`, paired = TRUE)
