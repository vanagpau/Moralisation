...
df2 %>% group_by(`Spreadsheet Row`, Question) %>% summarise(Mean = mean(Response), SE = signif(sd(Response) / sqrt(length(Response)),2)) %>% kbl() %>% kable_styling()

# Plot
df2 %>% ggplot(aes(x=Question, y=Response, fill=`Spreadsheet Row`)) + facet_wrap(df2$`Spreadsheet Row`) + geom_boxplot() + ggtitle("Familiarisation and Moralisation scores by issue Pre and Post stimulus") + ylab("Moral Score") + xlab("")

# paired t tests for moralisation score before and after

df2 %>% filter(`Spreadsheet Row` == "Self-driving cars", `Zone Name` != "slider") %>% group_by(Question, PID) %>% mutate(m = mean(Response)) %>% ungroup() %>% t.test(m ~ Question, data = ., paired = TRUE)
df2 %>% filter(`Spreadsheet Row` == "Meat", `Zone Name` != "slider") %>% group_by(Question, PID) %>% mutate(m = mean(Response)) %>% ungroup() %>% t.test(m ~ Question, data = ., paired = TRUE)
df2 %>% filter(`Spreadsheet Row` == "E-waste", `Zone Name` != "slider") %>% group_by(Question, PID) %>% mutate(m = mean(Response)) %>% ungroup() %>% t.test(m ~ Question, data = ., paired = TRUE)
df2 %>% filter(`Spreadsheet Row` == "Plastic", `Zone Name` != "slider") %>% group_by(Question, PID) %>% mutate(m = mean(Response)) %>% ungroup() %>% t.test(m ~ Question, data = ., paired = TRUE)
...
cohen_fn = function(var) {
  data.frame( df2 %>% filter(`Spreadsheet Row` == var, `Zone Name` != "slider")%>% group_by(Question, PID) %>% mutate(m = mean(Response)) %>% ungroup() %>% rstatix::cohens_d(., m ~ Question, paired = TRUE), Issue = var)
}

# run the calculations and produce tables
