...
df2 %>% group_by(`Spreadsheet Row`, Question) %>%
...
df2 %>% group_by(Question, PID, `Spreadsheet Row`) %>% mutate(m = mean(Response)) %>% select(PID, Question, m, `Spreadsheet Row`) %>% unique()  %>% ggplot(aes(x=`Spreadsheet Row`, y=m, fill=Question)) + geom_boxplot() + ggtitle("Familiarisation and Moralisation scores by issue Pre and Post stimulus") + ylab("Score") + xlab("") + theme(legend.position="bottom") 

# paired t tests for moralisation score before and after

df2 %>% filter(`Spreadsheet Row` == "Self-driving cars", `Zone Name` != "slider") %>%
  # select cars issue and ignore slider(familiarisation) questions
...
cohen_fn = function(var) {
  data.frame( df2 %>%
                ...
                filter(`Spreadsheet Row` == var, `Zone Name` != "slider") %>%
                # select issue and ignore slider(familiarisation) questions
                group_by(Question, PID) %>%
                # group by question (pre/post) and participant ID
                mutate(m = mean(Response)) %>%
                # create variable m which is mean of questionnaire items in pre and post for each participant 
