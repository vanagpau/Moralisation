...
df2 %>% filter(`Spreadsheet Row` == "Meat", `Zone Name` != "slider") %>%
  t.test(Response ~ Question, data = ., paired = TRUE)
...
cohen_fn = function(var) {
  data.frame(df2 %>% filter(`Spreadsheet Row` == var, `Zone Name` != "slider") %>% rstatix::cohens_d(., Response ~ Question, paired = TRUE), Issue = var)
}
