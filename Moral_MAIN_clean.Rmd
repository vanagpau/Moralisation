...
# Normality: Run Shapiro-Wilk test per variable mean per session and plot histograms of any distribution which is non-normal across all variables collected at all 4 time points (var_names4)
...
# save decriptives html table to file
describe(wide_df_all) %>% kbl(digits = 2, format = "html") %>% save_kable("descriptives.html")

# Show decriptives output
describe(wide_df_all)
...
summary(mm2)
performance::check_model(mm2, panel = FALSE)
...
summary(mm3)
performance::check_model(mm3, panel = FALSE)
AIC(mm3)

mm4 <- lmer(moral ~ emo + pig + cog + hed + (session|PID), data = df)
summary(mm4)
performance::check_model(mm4, panel = FALSE)
AIC(mm4)

mm5 <- lmer(moral ~ emo + pig + cog + att + (session|PID), data = df)
summary(mm5)
performance::check_model(mm5, panel = FALSE)
AIC(mm5)
...
lag1 <- lavaanPlot::lavaanPlot(name = "title", cross.1.fit, labels = labels1, coefs = TRUE, sig = 0.05, stars = c("regress"), edge_options = list(color = "grey"), graph_options = list(ratio = 0.5))
lag1
htmltools::save_html(lag1, file = "lag1.html")
...
lag2 <- lavaanPlot::lavaanPlot(name = "title", cross.2.fit, labels = labels2 ,coefs = TRUE, sig = 0.05, stars = c("regress"), edge_options = list(color = "grey"), graph_options = list(ratio = 0.75))
lag2
htmltools::save_html(lag2, file = "lag2.html")
...
labels3 = list(m1 = "Moralisation 1", m2 = "Moralisation 2", m3 = "Moralisation 3", m4 = "Moralisation 4",
...
lag3 <- lavaanPlot::lavaanPlot(name = "title", cross.3.fit, labels = labels3, coefs = TRUE, sig = 0.05, stars = c("regress"), edge_options = list(color = "grey"), graph_options = list(ratio = 0.75))
lag3
htmltools::save_html(lag3, file = "lag3.html")
